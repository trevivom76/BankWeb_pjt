<template>
  <div>
    <div class="box">
        ...
        <div class="input-container">
          <template v-if="selectedOption === '열심히 모은 목돈을 예치할 때'">
            ...
            <div class="input-group">
              <input type="text" class="input wide" placeholder="예치금액" v-model="price" />
              <span>원을</span>
            </div>

            <!-- 기간 -->
            <div class="input-group">
              <input type="text" class="input narrow" placeholder="기간" v-model="month" />
              <span>개월 간</span>
            </div>

            <!-- 금리 -->
            <div class="input-group">
              <input type="text" class="input narrow" placeholder="금리" v-model="percent" />
          ...
          <template v-if="selectedOption === '매월 일정금액을 저축할 때'">
            <!-- 예치금액 -->
            <div class="input-group">
              <input type="text" class="input wide" placeholder="월저축액" v-model="price" />
              <span>원씩</span>
            </div>
      ...
      <div v-if="totalPrice !== ''">
        <template v-if="selectedOption === '열심히 모은 목돈을 예치할 때'">
          <p class="result-text"><span class="bold-text">{{ price }}</span>원을 <span class="bold-text">{{ month
              }}</span>개월 간 <span class="bold-text">{{ percent }}</span>%의 예금상품에 저축하시면</p>
          <p class="result-text">이자 <span class="bold-text">{{ interest }}</span>원을 더해 총 <span
              class="bold-text bold-color">{{ totalPrice }}</span>원을 모으실 수 있습니다.</p>
        </template>
        <template v-if="selectedOption === '매월 일정금액을 저축할 때'">
          <p class="result-text"><span class="bold-text">{{ price }}</span>원을 <span class="bold-text">{{ month
              }}</span>개월 간 <span class="bold-text">{{ percent }}</span>%의 적금상품에 저축하시면</p>
          <p class="result-text">이자 <span class="bold-text">{{ interest }}</span>원을 더해 총 <span
              class="bold-text bold-color">{{ totalPrice }}</span>원을 모으실 수 있습니다.</p>
        </template>
        <template v-if="selectedOption === '목표금액을 모을 때'">
          <p class="result-text"><span class="bold-text">{{ price }}</span>원을 목표로 <span class="bold-text">{{ month
              }}</span>개월 간 <span class="bold-text">{{ percent }}</span>%로 저축하시면</p>
          <p class="result-text">이자 <span class="bold-text">{{ interest }}</span>원을 제하고 월 <span
              class="bold-text bold-color">{{ totalPrice }}</span>원씩 저축하시면 됩니다.</p>
        </template>
      </div>
    </div>
  </div>
