<template>
  <div class="finance-box">
    ...
    <div class="select-box">
      ...
      <!-- 예치 기간 선택 셀 -->
      <v-select label="예치기간" :items="['전체기간', 6, 12, 24, 36]" v-model="selectedSavingPeriod" variant="outlined"
    ...
    <!-- 정기 적금 테이블 -->
    <v-data-table-virtual :items="savingItems" :headers="headers">
      <template #item="{ item }">
  ...
  <v-dialog v-model="isModalVisible" width="800">
    <v-card v-if="selectedRowItem" class="card">
      ...
      <div>
        ...
        <v-table>
          <tbody>
            <tr v-for="(value, key) in selectedRowItem" :key="key">
              <template
                v-if="['금융 회사명', '금융 상품명', '6개월 자유', '6개월 정액', '12개월 자유', '12개월 정액', '24개월 자유', '24개월 정액', '36개월 자유', '36개월 정액', 'freeSavingData', 'freeSavingData2', 'freeSavingData3', 'fixedSavingData', 'fixedSavingData2', 'fixedSavingData3', '금융 상품 ID'].includes(key)"></template>
              <template v-else>
        ...
        <SavingChart :free-saving-data="selectedRowItem['freeSavingData']"
