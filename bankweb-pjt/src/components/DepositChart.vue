<template>
  <div class="chart-container">
    <ul class="chart-list">
      <li class="chart-item">
        <h3 class="chart-title">예치 기간별 우대 금리 비교</h3>
        <canvas ref="intrRateChart"></canvas>
      </li>
    </ul>
...
// 캔버스 엘리먼트를 참조할 ref 생성
const intrRateChart = ref(null);

onMounted(() => {
  // 기간 레이블
  const labels = ["6개월", "12개월", "24개월", "36개월"];

  const intrRate3Data = [0, 0, 0, 0]

  for (let i = 0; i < 4; i++) {
    intrRate3Data[i] = (prop.intrRateData[i] + prop.intrRate2Data[i]) / 2.0
  }


  if (intrRateChart.value) {
    new Chart(intrRateChart.value.getContext("2d"), {
      type: "line", // 라인 차트
      data: {
        labels,
        datasets: [
