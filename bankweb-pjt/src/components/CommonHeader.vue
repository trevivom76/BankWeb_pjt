<template>
  <div>
        ...
        <div class="text-center">
          ...
          <RouterLink :to="{ name: 'interestrate' }">
            <v-hover v-slot="{ isHovering, props }">
              <v-btn :class="{ 'hovered-btn': isHovering }" :elevation="isHovering ? 10 : 0" v-bind="props">금리비교</v-btn>
            </v-hover>
          </RouterLink>
          ...
          <RouterLink :to="{ name: 'currencycalculator' }">
            <v-hover v-slot="{ isHovering, props }">
              <v-btn :class="{ 'hovered-btn': isHovering }" :elevation="isHovering ? 10 : 0" v-bind="props">환율계산</v-btn>
            </v-hover>
          </RouterLink>
          ...
          <RouterLink :to="{ name: 'aroundbank' }">
            <v-hover v-slot="{ isHovering, props }">
              <v-btn :class="{ 'hovered-btn': isHovering }" :elevation="isHovering ? 10 : 0" v-bind="props">주변은행</v-btn>
            </v-hover>
          </RouterLink>
          ...
          <RouterLink :to="{ name: 'community' }">
            <v-hover v-slot="{ isHovering, props }">
              <v-btn :class="{ 'hovered-btn': isHovering }" :elevation="isHovering ? 10 : 0" v-bind="props">커뮤니티</v-btn>
            </v-hover>
          </RouterLink>
        </div>
      </div>
    ...
    <!-- 로그인 안내 다이얼로그 -->
    <v-dialog v-model="dialog" max-width="380" height="190" persistent>
      <v-card text="안전한 금융서비스를 위해 로그인 화면으로 이동합니다">
        <!-- 중앙 정렬을 위해 flexbox 사용 -->
        <v-card-text class="d-flex justify-center align-center mb-5 pt-0">
          <v-progress-circular color="primary" indeterminate disable-shrink size="40" width="4"></v-progress-circular>
        </v-card-text>
      </v-card>
    </v-dialog>
  </div>
