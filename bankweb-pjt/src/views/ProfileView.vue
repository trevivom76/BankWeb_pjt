    ...
    <div class="card">
      <div class="profile-links">
        <RouterLink 
          ...
          class="profile-link"
          :class="{ active: selectedCategory === 'profilemanage' }"
           @click="setSelectedCategory('profilemanage')"
        >
          회원정보 관리 페이지
        </RouterLink>
        <RouterLink 
          :to="{ name: 'contractedproduct' }" 
          class="profile-link"
...
.profile-links {
  display: flex;
  flex-direction: row; /* 버튼들을 세로로 정렬 */
  gap: 24px; /* 버튼 간 간격 */
  align-items: flex-start; /* 왼쪽 정렬 (가운데 정렬을 원하면 center로 변경) */
  margin-bottom: 24px;
}
...
.profile-link::after {
  ...
  height: 2px; /* 선 두께 */
  width: 100%;
  background-color: #5058cc; /* 선 색상 */
  transform: scaleX(0); /* 초기 상태에서 선 숨김 */
  transform-origin: left;
  transition: transform 0.3s ease; /* 선 애니메이션 */
}

/* 활성 상태의 링크 */
.profile-link.active::after {
  transform: scaleX(1); /* 선이 유지됨 */
}

.profile-link.active {
  color: #5058cc; /* 텍스트 색상 강조 */
}

/* 마우스 호버 */
.profile-link:hover {
  color: #5058cc; /* 텍스트 색상 변경 */
  transform: translateX(5px); /* 살짝 오른쪽으로 이동 */
}

.profile-link:hover::after {
  transform: scaleX(1); /* 선 애니메이션 활성화 */
}
...
.disabled-link::after {
  background-color: transparent; /* 비활성화 상태에서 선 제거 */
}
...
.scene {
  ...
  justify-content: center;
  align-items: center;
  margin: 0px auto 40px; /* 가운데 정렬 */
  position: relative; /* 그림자 배치를 위한 기준 */
}

/* 그림자 */
.shadow {
  position: absolute;
  bottom: -20px; /* 그림자를 컨테이너 아래로 배치 */
  width: 150px;
  height: 20px;
  background: radial-gradient(ellipse at center, rgba(0, 0, 0, 0.2), rgba(0, 0, 0, 0));
  border-radius: 50%;
  z-index: -1; /* 3D 객체 아래로 배치 */
  filter: blur(5px); /* 부드러운 그림자 효과 */
}
...
.cube {
  ...
  animation: spin 3s infinite linear; /* 좌우 회전 애니메이션 */
}
...
/* 앞면 배치 */
.front {
  transform: rotateY(0deg) /* 3D 공간에서 앞면 배치 */
}

/* 뒷면 배치 */
.back {
  transform: rotateY(180deg) /* 3D 공간에서 뒷면 배치 */
}
