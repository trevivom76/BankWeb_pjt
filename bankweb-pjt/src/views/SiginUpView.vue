<template>
  <div>
    ...
    <v-card class="mx-auto pa-12 pb-8" elevation="8" max-width="448" rounded="lg">
      ...
      <!-- 이름 입력 text-field -->
      <v-text-field class="my-2" v-model="name" color="primary" label="이름" placeholder="홍길동" variant="outlined"></v-text-field>

      <!-- 닉네임 입력 text-field -->
      <v-text-field class="my-2" v-model="nickname" color="primary" label="닉네임" placeholder="닉네임" variant="outlined"></v-text-field>
      ...
      <!-- 이메일 입력 text-field -->
      <v-text-field v-model="email" color="primary" label="이메일" variant="outlined" placeholder="(선택사항)"></v-text-field>
...
const signUp = function () {
  ...
  if (usernameIsOk.value && password1IsOk.value && password2IsOk.value && passwordsIsOk.value) {
    const payload = {
      // 아이디
