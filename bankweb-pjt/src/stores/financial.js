...
export const useFinancialStore = defineStore("financial", () => {
  ...
  const depositToggleLikeStatus = async (deposit_id) => {
    try {
      const response = await axios.put(`${API_URL}/api/v1/deposits/deposit/${deposit_id}/toggle-like/`, {}, {
        headers: {
  ...
  const savingToggleLikeStatus = async (saving_id) => {
    try {
      const response = await axios.put(`${API_URL}/api/v1/deposits/saving/${saving_id}/toggle-like/`, {}, {
        headers: {
          Authorization: `Token ${accountStore.token}` // 인증 토큰
        }
      });
